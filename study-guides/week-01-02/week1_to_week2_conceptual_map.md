...
```mermaid
flowchart TB
    subgraph W1["WEEK 1 — BUILD THE ANALYSIS METHOD"]
        A["Ethics is reasoned judgment<br/>not merely personal opinion"]
        B["Separate facts from assumptions"]
        C["Identify actor + precise act<br/>stakeholders + alternatives"]
        D["Apply a complete ethical test"]
        E["Reach a supported conclusion"]
        A --> B --> C --> D --> E
    end

    subgraph U["WEEK 1 — FIRST ETHICAL LENS"]
        F["UTILITARIANISM"]
        G["What consequences follow?"]
        H["Who is everyone affected?"]
        I["Which option maximizes<br/>pleasure over pain?"]
        J["Act or generally accepted rule?"]
        K["Limitations:<br/>measurement, subjectivity,<br/>distribution and minority harm"]
        F --> G --> H --> I --> J --> K
    end

    E --> F
    K --> L["Consequences are important—but are they everything?"]

    subgraph W2["WEEK 2 — EXPAND THE TOOLBOX"]
        M["KANT<br/>Intention, universal moral law<br/>and respect for persons"]
        N["EGOISM<br/>The actor's rational<br/>self-interest"]
        O["ARISTOTLE<br/>Character, voluntariness,<br/>virtue and the mean"]
        P["CONFUCIANISM<br/>Relationships, reciprocity<br/>and role-based duties"]
    end
    ...
    M --> Q["Compare theories on the same case"]
    N --> Q
    O --> Q
    P --> Q

    Q --> R["Explain why they agree or disagree"]
    R --> S["Give a fact-sensitive conclusion<br/>and practical recommendation"]
```
